...
<body>
	...
	<fieldset>
		...
		<%=posts.get(i).no%>
		<a
			href="/board/read?category=<%=blp.category%>&no=<%=posts.get(i).no%>"><%=posts.get(i).title%></a>
		<%=posts.get(i).id%>
		<hr>
		<%
		}
	...
	<fieldset>
		<legend>링크</legend>
		<a href="/write.jsp?category=<%=blp.category%>">쓰기</a> <a
			href="/board/list?category=<%=blp.category%>&">list로</a> <a href="/">홈으로</a>
	</fieldset>
	<fieldset>
		<legend>검색</legend>
